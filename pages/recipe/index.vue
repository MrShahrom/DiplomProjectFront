  ...
  methods: {
    ...
    async handleExport() {
      ...
      try {
        ...
        const response = await getRecipes(headers);
        ...
        if (response && response.data && response.data.data) {
          ...
          const tableData = response.data.data.map(project => ({
            'ID': project.id,
            'Название тип продукта': project.id_type_product.product_name,
            'Название сырье': project.id_raw_material.name,
            'Ед. измерения (кг)': project.unit,
          ...
          link.href = url;
          link.download = 'clients.xlsx'; // Указываем имя файла
