<template>
    <h2 class="text-center mt-5 mb-3">Изменить клиента</h2>
    <div class="card">
        <div class="card-header">
            <NuxtLink class="btn btn-outline-info float-right" to="/customer">Все клиенты</NuxtLink>
        </div>
        <div class="card-body">
            <form @submit.prevent="handleSave">
                ...
                <div class="form-group">
                    ...
                    <input v-model="project.firstname" type="text" class="form-control" id="firstname"
                        name="firstname" />
                </div>
                <div class="form-group">
                    <label for="patronymic">Отчествоч</label>
                    <input v-model="project.patronymic" type="text" class="form-control" id="patronymic"
                        name="patronymic" />
                </div>
                <div class="form-group">
...
import { getClient, updateClient } from '~/services/projectService';
    ...
    methods: {
        async fetchProduct(clientID) {
            try {
                ...
                const response = await getClient(clientID, headers);
        ...
        async handleSave() {
            try {
                ...
                await updateClient(this.project.id, this.project, headers);
                Swal.fire({
                    icon: 'success',
