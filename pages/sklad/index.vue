  ...
  <section>
    <div class="container2">
      ...
      <div class="card-body order-table">
        <NuxtLink to="/sklad/create" class="btn btn-primary btn-add">Добавить склада</NuxtLink>
        ...
        <table class="table table-bordered">
          ...
          <tbody>
            <tr v-for="project in projects" :key="project.id">
              ...
              <td>
                <NuxtLink :to="`/sklad/edit/${project.id}`" class="btn btn-outline-success mx-1">Изменить</NuxtLink>
              </td>
              <td>
...
import { getTySkladall, deleteSklad } from '~/services/projectService' // Обновленный импорт
  ...
  methods: {
    fetchProjectList() {
      if (process.client) {
        ...
        getTySkladall(headers)
          .then(response => {
            this.projects = response.data["data"];
          })
          .catch(error => {
            console.error('Error fetching sklads:', error);
          });
      } else {
    ...
    async handleExport() {
      ...
      try {
        ...
        const response = await getTySkladall(headers);
        ...
        if (response && response.data && response.data.data) {
          ...
          link.href = url;
          link.download = 'sklad.xlsx'; // Указываем имя файла
    ...
    async handleDelete(supplierId) {
      const confirmResult = await Swal.fire({
        title: 'Вы уверены?',
        text: 'Вы действительно хотите удалить этого склада?',
        icon: 'warning',
      ...
      if (confirmResult.isConfirmed) {
        ...
        try {
          await deleteSklad(supplierId, headers);
